    ...
    if (args != null && args['isEditing'] == true) {
      ...
      if (adData != null) {
        setState(() {
          ...
          _carbrandController.text = adData['carBrand'] ?? '';
          selectedCarModel = adData['year'];
          _priceController.text = adData['price']?.toString() ?? '';
          _mileageController.text = adData['mileage']?.toString() ?? '';
          _fuelController.text = adData['fuel'] ?? '';
          _descriptionController.text = adData['description'] ?? '';
          selectedLocation = adData['location'];
          selectedLocationAddress = adData['location'] ?? '';
          selectedRegisteredIn = adData['registeredIn'];
          _bodyColorController.text = adData['bodyColor'] ?? '';
        });
      }
    }
  }
...
//  this is added to test on both web and mobile =-=--=-=-=-=-=-=-=-=-=-=-=-==-=-=
        ...
        content: ListView(
          ...
          children: [
            const Text("Unregistered",
                style: TextStyle(
                    color: Color(0xFFf48c25), fontWeight: FontWeight.bold)),
            ListTile(
  ...
  Future<bool> _verifyVehicleDetails() async {
    try {
      ...
      print('Registration No: ${_registrationNoController.text.trim()}');
      print('Chassis No: ${_chassisNoController.text.trim()}');
      ...
      String normalize(String value, {bool isRegistrationNo = false}) {
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUser == null) {
      return Scaffold(
        appBar: AppBar(
          title: Text(_isEditing ? "Edit Your Car" : "Sell Your Car"),
                    ...
                    child: _images.isEmpty && _webImages.isEmpty
                        ? const Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.camera_alt_outlined,
                                    size: 30, color: Color(0xFFf48c25)),
                                SizedBox(height: 8),
                                Text("Add Photo",
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        const Row(
                          children: [
                            Icon(Icons.rotate_right, color: Color(0xFFf48c25)),
                            SizedBox(width: 8),
                            Text(
                              '360° View Photos',
                              style: TextStyle(
